...
/// 字元的行上並不相等。轉換由 orikagoc 邊車在協定邊界完成，因此本組件公開的所有欄號
/// 一律為 UTF-16 單位
/// </para>
/// <para>
...
public sealed class GoLocation
{
    ...
    /// </summary>
    /// <remarks>
    /// 不是 <c>go/token</c> 的行內位元組數
    /// </remarks>
    public int Column { get; }

    /// <summary>取得自檔案開頭起算的<em>位元組</em>位移</summary>
    /// <remarks>
    /// 來源未提供時為 0
    /// </remarks>
    public int Offset { get; }

    /// <summary>取得位置所屬的檔案路徑</summary>
    /// <remarks>
    /// 來源未提供時為 <see langword="null"/>
    /// </remarks>
    public string? File { get; }

    /// <summary>以 <c>file(line,col)</c> 形式傳回位置的文字表示</summary>
    /// <returns>位置的文字表示，數字一律以不因文化特性而異的格式輸出</returns>
    public override string ToString()
    {
        ...
        return string.Create(CultureInfo.InvariantCulture, $"{File}({Line},{Column})");
    }
}